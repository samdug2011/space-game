...
func _ready() -> void:
	...
	for i in range(star_count):
		...
		if r > 0.01:
			var sun = star.get_child(0)
			var planet = star.get_child(1)
			var G = 6.67430
			...
			sun.linear_velocity = tangential_dir * v_mag * (1 + (randf() - 0.5) * 0.1)
			planet.linear_velocity = tangential_dir * v_mag * (1 + (randf() - 0.5) * 0.1)
			var Planet_v_mag = sqrt(G * sun.mass / sun.transform.origin.distance_to(planet.transform.origin))
			planet.linear_velocity += Vector3(Planet_v_mag, 0, 0)
